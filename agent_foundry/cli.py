...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_new.add_argument("goal", help="Natural language goal, e.g. '我想做一个 SVN Review Agent'")
...
def _interactive_collect(session) -> None:
    ...
    for stage in stages:
        ...
        for q in board.questions:
            ...
            raw = input(f"{q.id} [{default}]: ").strip()
            if not raw:
                value = default
            ...
            for opt in q.options:
                if opt.id in selected_values and opt.requires_input:
                    ...
                    if supplemental:
                        inputs[opt.requires_input["id"]] = supplemental
