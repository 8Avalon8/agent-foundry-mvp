    ...
    if stage == "tool_permissions":
        return [
            ...
            DecisionQuestion(
                ...
                stage=stage,
                recommended=None,
                recommendation_reason="只有选择自动运行白名单测试时，才需要填写可自动执行的命令。",
                options=[],
                affects=["tool_policy.shell.run_tests.allowed_commands"],
                risk_level="medium",
                visible_when={"question": "run_tests", "equals": "allow_whitelist"},
                placeholder="例如：python3 -m unittest discover -s tests -v",
            ),
            DecisionQuestion(
                id="patch_policy",
                title="是否允许生成 patch？",
            ...
            DecisionQuestion(
                id="patch_write_strategy",
                title="Patch 写入前需要怎样确认？",
                input_type="single_choice",
                stage=stage,
                recommended="diff_review",
                recommendation_reason="只有允许生成 patch 或修改工作区时，才需要确认 patch 写入策略。",
                options=[
                    DecisionOption("diff_review", "先展示 diff，再由用户确认", recommended=True),
                    DecisionOption("file_by_file", "逐文件确认"),
                    DecisionOption("single_approval", "整批一次确认"),
                ],
                affects=["human_feedback.before_patch_write", "tool_policy.fs.write_patch"],
                risk_level="medium_high",
                visible_when={"question": "patch_policy", "in": ["write_patch_ask", "modify_workspace_ask"]},
            ),
            DecisionQuestion(
...
def _writing_questions(stage: str) -> List[DecisionQuestion]:
    ...
    if stage == "tool_permissions":
        return [
            ...
            DecisionQuestion(
                id="publish_approval_detail",
                title="发布审批卡需要包含哪些信息？",
                input_type="multi_choice",
                stage=stage,
                recommended=["title", "summary", "risk", "final_content"],
                recommendation_reason="只有允许同步草稿或发布时，才需要设计发布审批卡细节。",
                options=[
                    DecisionOption("title", "标题", recommended=True),
                    DecisionOption("summary", "摘要", recommended=True),
                    DecisionOption("risk", "发布风险", recommended=True),
                    DecisionOption("final_content", "最终正文预览", recommended=True),
                    DecisionOption("account", "目标账号"),
                ],
                affects=["human_feedback.before_publish.approval_card"],
                risk_level="critical",
                visible_when={"question": "publish_policy", "in": ["explicit_confirmation", "draft_to_platform"]},
            ),
        ]
    if stage == "feedback_protocol":
        return [
